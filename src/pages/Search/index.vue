                ...
                <li :class="{ active: isOne }" @click="changeOrder('1')">
                  <a
                    >综合<span
                      ...
                      class="iconfont"
                      :class="{
                        'icon-jiantou_xiangshang_o': isAsc,
                        'icon-jiantou_xiangxia_o': isDesc,
                      }"
                    ></span
                  ></a>
                </li>
...
export default {
  ...
  methods: {
    ...
    changeOrder(flag) {
      ...
      //这里获取到的是最开始的状态
      let origonFlag = this.searchParams.order.split(":")[0];
      let origonSort = this.searchParams.order.split(":")[1];
      // 准备一个新的order属性值
      let newOrder = "";
      // 点击的是综合
      if (flag == origonFlag) {
        newOrder = `${origonFlag}:${origonSort == "desc" ? "asc" : "desc"}`;
      } else {
        // 点击的是价格
        newOrder = `${flag}:${"desc"}`;
      }
      // 匠心的order赋予searchParams
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 再起发起ajax请求
      this.getData();
      // 每一次请求完毕，应该吧相应的1 、2 、3级分类的id置空的，让他接受下一次的相应1 、2 、3级分类的id
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
    },
